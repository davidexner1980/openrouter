...
class AgentTaskExecutor internal constructor(
    ...
    suspend fun executeOneTask(
        ...
        val routingProfile = AgentRoutingPolicy.profileForGoal(freshGoal)
        val councilModelId = AgentCouncilPolicy.selectModel(councilRole, routingProfile, freshGoal.executionModelId)

        // EVIDENCE CAPABILITY GATE: V42-A.1 - Move before state mutation
        if (freshTask.capability in AgentCapability.SOURCE_DEPENDENT_FACTUAL_CAPABILITIES) {
            val gaps = AgentResearchAllocator.evaluateGaps(freshGoal, allocationProfile)
